# -------------------------------------------------------------------------------
# QGIS-FME Form Connector - Version 4.0.0
# -------------------------------------------------------------------------------
#
...
# Contact: sales@gis.fm
#
# Copyright 2026 GIS Innovation Sdn Bhd. All rights reserved.
# -------------------------------------------------------------------------------
...
from qgis.PyQt.QtWidgets import (
    ...
    QLineEdit, QHBoxLayout, QTreeView, QSplitter, QDialog, QFrame,
    QStyledItemDelegate, QScrollArea, QProgressBar, QPlainTextEdit
)
from qgis.PyQt.QtCore import Qt, QCoreApplication, QVariant, QEvent, pyqtSignal, QUrl, QTimer, QDir
from qgis.PyQt.QtGui import QDesktopServices, QFileSystemModel
...
class FMEFileLister(QWidget):
    ...
    def build_ui(self):
        ...
        self.file_model.setFilter(
            QDir.Filter.AllDirs | QDir.Filter.Files | QDir.Filter.Drives | QDir.Filter.NoDotAndDotDot | QDir.Filter.Hidden
        )
        ...
        self.source_dataset_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.source_dataset_table.setSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum)
        ...
        self.source_dataset_table.setEditTriggers(QAbstractItemView.EditTrigger.DoubleClicked | QAbstractItemView.EditTrigger.EditKeyPressed)
        ...
        self.dest_dataset_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.dest_dataset_table.setSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum)
        ...
        self.dest_dataset_table.setEditTriggers(QAbstractItemView.EditTrigger.DoubleClicked | QAbstractItemView.EditTrigger.EditKeyPressed)
        ...
        self.user_parameters_table.horizontalHeader().setSectionResizeMode(QHeaderView.ResizeMode.Stretch)
        self.user_parameters_table.setSizePolicy(QSizePolicy.Policy.Minimum, QSizePolicy.Policy.Minimum)
        ...
        self.user_parameters_table.setEditTriggers(QAbstractItemView.EditTrigger.DoubleClicked | QAbstractItemView.EditTrigger.EditKeyPressed)
        ...
        self.paths_table.item(0, 0).setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsEditable)
        self.paths_table.item(0, 1).setFlags(Qt.ItemFlag.ItemIsEnabled | Qt.ItemFlag.ItemIsEditable)
        ...
        about_text.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse | Qt.TextInteractionFlag.LinksAccessibleByMouse)
        ...
        about_content = f"""
            <div style='text-align: center; margin-bottom: 20px;'>
                ...
                <p style='color: #7f8c8d; margin-top: 0;'>Version 4.0.0</p>
            </div>
            ...
            <p style='margin-bottom: 15px;'>
                ...
                <ul style='margin-left: 20px;'>
                    ...
                    <li>Automated GeoJSON data handling</li>
            ...
            <p style='margin-bottom: 15px;'>
                ...
                <b>Contact:</b> <a href='mailto:sales@gis.fm' style='color: #3498db; text-decoration: none;'>sales@gis.fm</a>
            </p>

            <p style='color: #95a5a6; font-size: 9pt; text-align: center; margin-top: 30px;'>
                2026 GIS Innovation Sdn Bhd. All rights reserved.
            </p>
        """
    ...
    def navigate_to_address(self):
        ...
        if os.path.exists(path) and os.path.isdir(path):
            ...
            self.directory_selected.emit(path)  # Emit the signal
        else:
    ...
    def on_tree_item_clicked(self, index):
        ...
        if norm_path.lower().endswith('.fmw'):
            ...
            # First load the FMW info which will populate the dataset tables
            self.load_fmw_info(norm_path)
    ...
    def load_fmw_info(self, fmw_path):
        """Load and parse the FMW file content."""
        try:
            # Set the loading flag to prevent FMECommandLine from being called
            self.is_loading_fmw = True
            ...
            self.current_file = fmw_path
            
            # Check workspace compatibility
            is_compatible, compatibility_message = self.check_workspace_compatibility(fmw_path)
            ...
            for index, line in enumerate(lines):
                ...
                if header_started:
                    ...
                    if "Command line to run this workspace:" in line:
                        command_line_found = True
                        # Collect all lines that are part of the command line
                        command_lines = []
                        i = index + 1
                        while i < len(lines) and lines[i].strip().startswith("#"):
                            command_line_part = lines[i].strip().lstrip("#").strip()
                        ...
                        # Use regex to extract paths within quotes or contiguous non-space characters
                        match = re.findall(r'"([^"]+)\'|\'([^\']+)\'|(\S+)', command_line)
                        # Flatten the match results
                        parts = [m[0] or m[1] or m[2] for m in match]
                        if len(parts) >= 2:
                            # Find the FME executable path (it's the first quoted path)
                            fme_path = None
                            workspace_path = None
                            
                            # Join parts until we find both paths
                            current_path = []
                            in_quotes = False
                            ...
                            if fme_path and workspace_path:
                                # Update paths table - keep quotes for these paths
                                # First ensure the table has items
                                if not self.paths_table.item(0, 0):
                                    self.paths_table.setItem(0, 0, QTableWidgetItem(""))
                                if not self.paths_table.item(0, 1):
                                    self.paths_table.setItem(0, 1, QTableWidgetItem(""))
                                
                                # Now set the values
                                self.paths_table.item(0, 0).setText(fme_path)
                                self.paths_table.item(0, 1).setText(workspace_path)
            ...
            QMessageBox.critical(self, "Error", f"An error occurred while loading the FMW file:\n{str(e)}\n\nDetails:\n{error_details}")
    ...
    def select_fme_exe(self, row, column):
        ...
        try:
            ...
            if file_path:
                ...
                # Update the FME executable path in the paths table with quotes
    ...
    def select_workspace(self, row, column):
        ...
        try:
            ...
            if file_path:
                ...
                # Load the FMW info which will also update the FME path
                self.load_fmw_info(file_path)
    ...
    def update_command_display(self, item=None):
        ...
        if command_text:
            ...
            if current_command:
                ...
                command_text.setPlainText(shlex.join(current_command))
            else:
    ...
    def build_fme_command(self):
        ...
        # Check if we have a valid FMW file
        if not self.current_file or not os.path.exists(self.current_file):
            return None
            
        # Get FME executable path
        fme_exe_path = "C:/Program Files/FME/fme.exe"  # Default FME path
        
        # Build command as a list of arguments (safe for subprocess with shell=False)
        command_parts = [fme_exe_path, self.current_file]
        ...
        for row in range(self.user_parameters_table.rowCount()):
            ...
            if param_name_item and param_value_item and param_value_item.text():
                ...
                command_parts.append(param_value)
        
        # Add source and destination dataset parameters if they exist
        if self.source_dataset_table and self.source_dataset_table.rowCount() > 0:
            source_item = self.source_dataset_table.item(0, 1)
            if source_item and source_item.text():
                source_value = source_item.text().strip('"')
                command_parts.append('--SourceDataset_GEOJSON')
                command_parts.append(source_value)
        
        if self.dest_dataset_table and self.dest_dataset_table.rowCount() > 0:
            dest_item = self.dest_dataset_table.item(0, 1)
            if dest_item and dest_item.text():
                dest_value = dest_item.text().strip('"')
                command_parts.append('--DestDataset_GEOJSON')
                command_parts.append(dest_value)
    ...
    def create_command_execution_panel(self, command, source_path, dest_path):
        ...
        try:
            ...
            # Keep command as list for execution, make a display string for the UI
            ...
            source_label.setCursor(Qt.CursorShape.IBeamCursor)
            ...
            dest_label.setTextInteractionFlags(Qt.TextInteractionFlag.TextSelectableByMouse)
            dest_label.setCursor(Qt.CursorShape.IBeamCursor)
    ...
    def execute_fme_command(self, command, source_path, dest_path):
        ...
        try:
            ...
            # Create process (shell=False for security)
                        ...
                        if os.path.exists(dest_path):
                            ...
                            if self.scratch_layer_checkbox.isChecked():
                                ...
                                if not source_layer.isValid():
                                    ...
                                    # Zoom to the layer if available
                                    try:
...
class QGISFMEConnectorDialog(QDialog):
    _instance = None  # Singleton instance for the dialog

    @classmethod
    def show_dialog(cls):
        if cls._instance is None:
            cls._instance = QGISFMEConnectorDialog()
        cls._instance.show()
    ...
    def create_command_execution_panel(self, command, source_path, dest_path):
        ...
        try:
            ...
            self.command_text.setPlainText(display_command)
    ...
    def execute_fme_command(self, command, source_path, dest_path):
        ...
        try:
            ...
            # Extract source and destination paths from the command list
            source_path = None
            dest_path = None
            for i, arg in enumerate(fme_command):
                if arg == '--SourceDataset_GEOJSON' and i + 1 < len(fme_command):
                    source_path = fme_command[i + 1]
                elif arg == '--DestDataset_GEOJSON' and i + 1 < len(fme_command):
                    dest_path = fme_command[i + 1]
            ...
            if not source_path:
                ...
                # Add to command
                fme_command.extend(['--SourceDataset_GEOJSON', source_path])
            ...
            if not dest_path:
                ...
                # Add to command
                fme_command.extend(['--DestDataset_GEOJSON', dest_path])
            ...
            self.command_text.setPlainText(shlex.join(fme_command))
...
# Show the dialog
QGISFMEConnectorDialog.show_dialog()